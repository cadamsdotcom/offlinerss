...
function createKvStore({ keyPrefix = '', ttlMs } = {}) {
  ...
  return {
    ...
    // `ttlMs` overrides the store-wide default expiry for this key only; omit
    // it to use the ttl passed to createKvStore.
    async set(key, value, ttlMs) {
      if (!enabled) return;
      try {
        const args = ['SET', keyPrefix + key, value];
        const sec = ttlMs ? Math.max(1, Math.round(ttlMs / 1000)) : ttlSec;
        if (sec) args.push('EX', String(sec));
        await command(args);
      } catch (err) {
