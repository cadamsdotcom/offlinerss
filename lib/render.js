...
function cleanBody(html, pageUrl) {
  ...
  // Make embedded media (YouTube/Vimeo iframes, <video>/<audio>) load in a
  // reader. Readability keeps allowed video embeds but its relative-URL fixup
  // only rewrites <a>/<img>/<video>, never <iframe> — and even that fixup is
  // skipped here because we parse without a base URL. So an embed's src commonly
  // reaches the reader still relative or protocol-relative (e.g.
  // src="//www.youtube.com/embed/…"), which then resolves against the reader's
  // own (file/about:blank) origin and silently fails — the "videos never work"
  // symptom that looks like a cross-origin problem. Absolutize every media src
  // (and poster) against the real page URL, promoting protocol-relative to
  // https, and cap the width so fixed-size embeds don't overflow the column.
  const absMediaAttr = ($el, attr) => {
    const cur = $el.attr(attr);
    if (!cur) return;
    const abs = absolutize(cur, pageUrl);
    if (abs) $el.attr(attr, abs);
  };
  $('iframe, video, audio, source, embed').each((_, el) => {
    const $el = $(el);
    // Lazy-loaded embeds park the real URL in data-src until scripts run (which
    // they never do in a reader); promote it so the embed has a usable src.
    if (!$el.attr('src') && $el.attr('data-src')) $el.attr('src', $el.attr('data-src'));
    absMediaAttr($el, 'src');
    absMediaAttr($el, 'poster');
  });
  $('iframe, video').each((_, el) => {
    $(el).attr('style', `max-width:100%;${$(el).attr('style') || ''}`);
  });
  $('pre').each((_, p) => {
