...
function renderComment(node, theme, pageUrl, parentId, rootId) {
  ...
  // Nesting is conveyed by indentation (plus the parent/root jumps), so no left
  // bars are needed; this keeps the most width for content.
  const blockStyle =
    node.depth === 0 ? `margin:14px 0 0 0;` : `margin:10px 0 0 ${indent}px;`;
