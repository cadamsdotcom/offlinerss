...
function renderComment(node, theme, pageUrl, parentId, rootId) {
  ...
  // Root comments drop the left bar/padding (it would otherwise run full-height
  // down the far left past the whole thread) to free up width; replies keep
  // their bars to show nesting.
  const blockStyle =
    node.depth === 0
      ? `margin:14px 0 0 0;`
      : `margin:10px 0 0 ${indent}px;border-left:3px solid ${theme.rule};padding:1px 0 1px 9px;`;
