...
async function renderDiscussion(pageUrl, feedKey, story = {}) {
  ...
  for (const cm of comments) {
    ...
    // Differentiate the byline with an accent underline + tint. Readers like
    // NetNewsWire strip background-color (to enforce their theme) but keep
    // borders and font-weight, so the underline is what actually shows there;
    // the tint is a bonus for readers that do honor backgrounds.
    out.push(
      `<div style="font-size:12px;color:${theme.byline};background:${theme.bylineBg};` +
        `border-bottom:1px solid ${theme.rule};padding:2px 6px 3px;margin-bottom:7px;">${meta}${reply}</div>`
    );
    if (cm.bodyHtml) {
      out.push(
