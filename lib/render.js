...
// Per-tier fetch budgets for article extraction, kept tight so no single
// article can approach the platform's function timeout (Vercel maxDuration:
// 60s). Logs showed bot-walled articles spending ~15s in the Jina fallback
// alone — stack a few across the 3 concurrency slots and a build sails past
// 60s (FUNCTION_INVOCATION_TIMEOUT). Worst case per article is now
// direct + jina + archive ≈ 24.5 + 12 + 8 ≈ 45s, and a failed article is
// negative-cached (see server.js), so this budget is spent at most once per
// article until its short TTL. The discussion and source-feed fetches keep the
// longer fetchWithTimeout defaults (with retries) on purpose: they hit
// HN/Lobsters, which rate-limit datacenter IPs and benefit from backoff.
const ARTICLE_DIRECT_TIMEOUT_MS = 12000;
const ARTICLE_DIRECT_RETRIES = 1;
...
async function fetchWithTimeout(url, timeoutMs = 20000, retries = 2, extraHeaders = {}) {
  let lastErr;
...
async function tryJinaReader(articleUrl) {
  ...
  try {
    ...
    const res = await fetchWithTimeout(`https://r.jina.ai/${articleUrl}`, JINA_TIMEOUT_MS, 0, headers);
...
async function tryArchive(articleUrl) {
  try {
    const res = await fetchWithTimeout(archiveUrl(articleUrl), ARCHIVE_TIMEOUT_MS, 0);
    const html = await res.text();
...
async function renderArticle(articleUrl, feedKey, story = {}) {
  ...
  try {
    const res = await fetchWithTimeout(articleUrl, ARTICLE_DIRECT_TIMEOUT_MS, ARTICLE_DIRECT_RETRIES);
    const html = await res.text();
    if (res.ok && !looksLikeBotWall(res.status, html)) {
