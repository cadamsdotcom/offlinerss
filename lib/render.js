...
async function renderDiscussion(pageUrl, feedKey, story = {}) {
  ...
  const comments = feedKey === 'lobsters' ? parseLobsters($) : parseHN($);
  ...
  for (const cm of comments) {
    ...
    const meta = [cm.author, cm.score ? `${cm.score} points` : null, cm.age]
      .filter(Boolean)
      .map(esc)
      .join(' · ');
    out.push(
    ...
    out.push(`<div style="font-size:12px;color:${theme.byline};margin-bottom:3px;">${meta}</div>`);
    if (cm.bodyHtml) {
      out.push(
...
module.exports = { renderDiscussion, fetchWithTimeout };
