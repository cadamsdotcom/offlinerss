...
function renderComment(node, theme, pageUrl, parentId, rootId) {
  ...
  // In-page jumps to ancestors (like HN's parent/root). They rely on the source
  // comment ids surviving as DOM ids; if a reader strips ids, the links are
  // simply inert. "root" is omitted when it would duplicate "parent".
  let nav = node.replyUrl ? link('reply', node.replyUrl) : '';
  if (parentId) nav += link('parent', `#c-${encodeURIComponent(parentId)}`);
  if (rootId && rootId !== parentId) nav += link('root', `#c-${encodeURIComponent(rootId)}`);
