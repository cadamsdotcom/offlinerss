...
// Readability + cleanBody pipeline as the direct path. Opt-in via JINA_READER=1
// (an optional JINA_API_KEY raises the rate limit but isn't required), so the
// happy path stays fully self-contained and no URLs leave our infra unless
// explicitly enabled. Returns { article, url } or null.
async function tryJinaReader(articleUrl) {
  if (process.env.JINA_READER !== '1') return null;
  try {
