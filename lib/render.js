...
function renderComment(node, theme, pageUrl, parentId, rootId, prevId, nextId) {
  ...
  // Styles are kept minimal (just the accent color) because each link's markup
  // repeats per comment — trimming it meaningfully shrinks the raw feed.
  const extLink = (label, href) =>
    ` · <a href="${esc(href)}" style="color:${theme.accent}">${label}</a>`;
