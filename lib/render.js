...
function renderComment(node, theme, pageUrl) {
  const esc = escapeHtml;
  // Indent is applied per level; DOM nesting makes it cumulative. Stop adding
  // past MAX_INDENT_DEPTH so deep threads don't run off a narrow screen.
  const indent = node.depth >= 1 && node.depth <= MAX_INDENT_DEPTH ? INDENT_PX : 0;
  const meta = [
    ...
    .filter(Boolean)
    .join(' · ');
  const reply = node.replyUrl
    ? ` · <a href="${esc(node.replyUrl)}" style="color:${theme.accent};text-decoration:none;">reply</a>`
    : '';
  ...
  const blockStyle = `margin:10px 0 0 ${indent}px;border-left:3px solid ${theme.rule};padding:1px 0 1px 9px;`;
  ...
  if (node.children.length > 0) {
    // A comment with replies becomes a native disclosure widget: toggling the
    // byline collapses this comment's body and its entire reply subtree. This
    // needs no JavaScript (feed readers run none); if a reader strips
    // <details>, the content simply stays expanded.
    const kids = node.children.map((c) => renderComment(c, theme, pageUrl)).join('');
    return (
      `<details open style="${blockStyle}">` +
      `<summary style="cursor:pointer;${bylineStyle}">${meta}${reply}</summary>` +
      body +
      kids +
      `</details>`
    );
  }
  return `<div style="${blockStyle}"><div style="${bylineStyle}">${meta}${reply}</div>${body}</div>`;
}
