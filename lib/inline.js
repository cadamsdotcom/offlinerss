...
async function buildSelfContainedPage(pageUrl, opts = {}) {
  ...
  // Feed content must be an HTML *fragment*, not a full document: readers drop
  // it into their own template, and a nested <head> (and the <style> it holds)
  // is the part most likely to be discarded. So append our override stylesheet,
  // then emit the stylesheets hoisted to the top of the body fragment
  // (style-in-body is valid HTML and still applies) followed by the body markup.
  $('head').append(`<style>${OVERRIDE_CSS}</style>`);

  const styleEls = $('style').toArray();
  const styles = styleEls.map((el) => $.html(el)).join('\n');
  $('style').remove();
