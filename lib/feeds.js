 ...
 * the target article. Self/text posts (Ask HN, Lobsters discussions) point
 * `link` back at the HN/Lobsters page — the same as `commentsUrl` — which the
 * builder detects to emit a single discussion item instead of the split.
 * `commentsUrl` is the HN/Lobsters discussion page.
 *
 * Each feed is built by querying its source once; each external-link source
 * story yields two output items, in original order: the article (reader mode)
 * and its discussion (self-posts yield just the discussion). `idFromComments`
 * gives each story a stable, human-readable id that the builder namespaces
 * (`article:`/`comments:`) into per-view cache keys so the two renderings of a
 * story never collide.
 */
const FEEDS = {
  hn: {
